...
extension UIView {
  func matchParent(padding: UIEdgeInsets = .zero) {
    ...
    if let superTopAnchor = superview?.safeAreaLayoutGuide.topAnchor {
      self.safeAreaLayoutGuide.topAnchor.constraint(equalTo: superTopAnchor, constant: padding.top).isActive = true
    }
    if let superBottomAnchor = superview?.safeAreaLayoutGuide.bottomAnchor {
      self.safeAreaLayoutGuide.bottomAnchor.constraint(equalTo: superBottomAnchor, constant: -padding.bottom).isActive = true
    }
    if let superLeadingAnchor = superview?.safeAreaLayoutGuide.leadingAnchor {
      self.safeAreaLayoutGuide.leadingAnchor.constraint(equalTo: superLeadingAnchor, constant: padding.left).isActive = true
    }
    if let superTrailingAnchor = superview?.safeAreaLayoutGuide.trailingAnchor {
      self.safeAreaLayoutGuide.trailingAnchor.constraint(equalTo: superTrailingAnchor, constant: -padding.right).isActive = true
    }
  }
