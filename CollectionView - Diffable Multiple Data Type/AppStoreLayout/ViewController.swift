...
class ViewController: UIViewController {
  ...
  func createLayout() -> UICollectionViewLayout {
    ...
    let layout = UICollectionViewCompositionalLayout { (sectionIndex, layoutEnvironment) -> NSCollectionLayoutSection? in
      ...
      headerItem.contentInsets = .init(top: 0, leading: 4, bottom: 0, trailing: 4)
      
      // displayScale -> 1.0, 2.0, 3.0
      ...
      topLineItem.contentInsets = .init(top: 0, leading: 4, bottom: 0, trailing: 4)
      ...
      bottomLineItem.contentInsets = .init(top: 0, leading: 4, bottom: 0, trailing: 4)
      ...
      case .categories:
        ...
        let itemLeadingAndTrailingInset = halfOfRemainingWidth + itemInset
        
        item.contentInsets = .init(top: 0, leading: itemLeadingAndTrailingInset, bottom: 0, trailing: itemLeadingAndTrailingInset)
  ...
  func configureDataSource() {
    dataSource = .init(collectionView: collectionView, cellProvider: { (collectionView, indexPath, item) -> UICollectionViewCell? in
      ...
      case .standard:
        ...
        let isThirdItem = (indexPath.item + 1).isMultiple(of: 3)
        cell.configureCell(item.app!, hideBottomLine: isThirdItem)
        
        return cell
      case .categories:
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CategoryCollectionViewCell.reuseIdentifier, for: indexPath) as! CategoryCollectionViewCell
        let isLastItem = collectionView.numberOfItems(inSection: indexPath.section) == indexPath.item + 1
